import { ArrowRight, Globe, Mail, MapPin, MessageCircle, Phone, Share2, Sparkles } from 'lucide-react'
...
function Footer() {
	return (
		<footer className="mt-16 border-t border-white/10 bg-gradient-to-b from-slate-950 to-slate-950/95 text-white">
			<div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
				<div className="grid gap-10 lg:grid-cols-[1.2fr_0.8fr_0.8fr_1fr]">
					<div>
						<Link to="/" className="inline-flex items-center gap-3 group">
							<span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-amber-300 to-orange-500 text-slate-950 shadow-lg shadow-amber-500/30 transition duration-200 group-hover:scale-105">
								<Sparkles className="h-5 w-5" />
							</span>
							<span>
								<span className="block text-xs font-semibold uppercase tracking-[0.28em] text-amber-300">Home Haven</span>
								<span className="block text-lg font-bold text-white">Living Essentials</span>
							</span>
						</Link>
					...
					<div>
						...
						<form className="mt-5 space-y-3">
							...
							<button
								type="submit"
								className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-amber-300 to-amber-400 px-4 py-3 text-sm font-bold text-slate-950 transition hover:-translate-y-0.5 hover:shadow-lg hover:shadow-amber-500/30"
							>
								Subscribe
